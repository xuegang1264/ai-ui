...
name: create-skill
description: >-
  Guides users through creating effective Agent Skills for Claude Code under
  ~/.claude/skills or <project>/.claude/skills. Use when the user wants to create,
  write, or author a new skill, or asks about skill structure, best practices,
  or SKILL.md format.
---
# Creating Skills in Claude

This skill guides you through creating effective Agent Skills for **Claude Code**. Skills are markdown directories with a `SKILL.md` that teach the agent how to perform specific tasks: reviewing PRs using team standards, generating commit messages in a preferred format, querying database schemas, or any specialized workflow.
...
**IMPORTANT**: Store authored skills only under the standard `skills/` trees: `~/.claude/skills/skill-name/` (user) or `<project-root>/.claude/skills/skill-name/` (repository). Do not place them alongside other toolchain-managed `.claude` internals unless product documentation explicitly says to.
